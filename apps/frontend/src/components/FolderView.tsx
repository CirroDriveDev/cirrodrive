...
import { EntryTableWithActions } from "#components/EntryDataTable.js"
import { EntryBulkActionToolbar } from "#components/EntryBulkActionToolbar.js"
import { useTrashMany } from "#services/file/useTrashMany.js"
...
export function FolderView({ folderId }: FolderViewProps): JSX.Element {
  ...
  const { createFolder, setParentFolderId } = useFolderCreate({
    onSuccess: (data) => setFolderId(data.id),
  })
  ...
  const { handleTrashMany } = useTrashMany()
  ...
  const handleDeleteCheckedFiles = async () => {
    const entriesToTrash = sortedEntries
      .filter((entry) => checkedFileList.some((f) => f.fileId === entry.id))
      .map((entry) => ({ id: entry.id, type: entry.type }))

    try {
      await handleTrashMany(entriesToTrash)
  ...
  return (
    <div className="flex w-full flex-grow flex-col items-center">
      ...
      <div className="flex h-16 w-full items-center space-x-4 p-4">
        ...
        {folderPathQuery.data?.slice(1).slice(-2).map((path) => (
          <div
      ...
      <div className="flex w-full items-center space-x-4 px-4 pb-4">
        ...
        <Button onClick={createFolder} className="flex items-center justify-between">
          <PlusIcon />
          <span>폴더</span>
        </Button>
      </div>
      ...
      {checkedFileList.length > 0 && (
        <div className="w-full px-4 pb-2">
          <EntryBulkActionToolbar
            ...
            onMove={() => {}}
            onDelete={handleDeleteCheckedFiles}
          />
        </div>
      )}
