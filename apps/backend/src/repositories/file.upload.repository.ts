...
import { Prisma, FileMetadata } from "@cirrodrive/database";
...
export class FileUploadRepository extends BaseRepository {
  // 파일 메타데이터 생성
  public async create(data: Prisma.FileMetadataUncheckedCreateInput): Promise<FileMetadata> {
  ...
  public async getByKey(key: string): Promise<FileMetadata> {
    const result = await this.prisma.fileMetadata.findUnique({ where: { key } });
    if (!result) {
