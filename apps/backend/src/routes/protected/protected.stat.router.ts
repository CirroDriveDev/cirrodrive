...
export const protectedStatRouter = router({
  ...
  getTotalFiles: adminProcedure.query(async ({ ctx }) => {
    ...
    logger.info({ requestId: ctx.req.id }, "protected.getTotalFiles 요청 성공");
    return { totalFiles };
  }),
  ...
  getTotalUsers: adminProcedure.query(async ({ ctx }) => {
    logger.info({ requestId: ctx.req.id }, "protected.getTotalUsers 요청 시작");
    const totalUsers = await adminService.getTotalUsers();
    logger.info({ requestId: ctx.req.id }, "protected.getTotalUsers 요청 성공");
    return { totalUsers };
  }),
